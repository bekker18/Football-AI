...
def pkg_versions() -> dict:
    out = {}
    for p in ("torch", "ultralytics", "supervision", "transformers",
              "umap-learn", "scikit-learn", "opencv-python-headless", "numpy"):
        try:
...
def main() -> None:
    ...
    ap.add_argument("--model-dir", default="/data", help="dir holding the .pt checkpoints")
    ...
    ap.add_argument("--stride-fit", type=int, default=60,
                    help="frame stride for collecting crops to fit the team classifier")
    ap.add_argument("--max-frames", type=int, default=0, help="0 = whole video (else cap)")
    ...
    print(f"[info] {args.source}  {info.width}x{info.height} @ {fps:.3f}fps  "
    ...
    for frame in tqdm(sv.get_video_frames_generator(args.source, stride=args.stride_fit),
                      desc="crops"):
        res = player_model(frame, imgsz=args.imgsz, verbose=False)[0]
        det = sv.Detections.from_ultralytics(res)
        crops += get_crops(frame, det[det.class_id == PLAYER_CLASS_ID])
    if len(crops) < 8:
        raise SystemExit(f"Only {len(crops)} player crops found — clip too short/empty "
    ...
    if args.save_video:
        ...
        ellipse = sv.EllipseAnnotator(color=sv.ColorPalette.from_hex(colors), thickness=2)
        ...
        def _radar(merged, lookup, transformer):
            ...
            for i, hexc in enumerate(colors):
                radar = draw_points_on_pitch(config=CONFIG, xy=txy[lookup == i],
    ...
    for fidx, frame in enumerate(gen):
        ...
        players_team = team_classifier.predict(get_crops(frame, players)) \
            if len(players) else np.array([], dtype=int)
        ...
        def emit(role, team, oid, xyxy):
            ...
            row = dict(frame=fidx, time_s=round(time_s, 4), object_id=oid, role=role,
        ...
        for i in range(len(players)):
            ...
            emit("player", players_team[i] if i < len(players_team) else None,
                 tid, players.xyxy[i])
        for i in range(len(goalkeepers)):
            tid = int(goalkeepers.tracker_id[i]) if goalkeepers.tracker_id is not None else -1
            emit("goalkeeper", gk_team[i] if i < len(gk_team) else None,
                 tid, goalkeepers.xyxy[i])
        ...
        frames_jsonl.append(dict(frame=fidx, time_s=round(time_s, 4),
                                 pitch_valid=transformer is not None,
                                 objects=frame_objs))

        if args.save_video:
            merged = sv.Detections.merge([players, goalkeepers, referees])
            lookup = np.array(players_team.tolist() + gk_team.tolist() +
                              [2] * len(referees))  # 2 == referee color
            ...
            rh, rw, _ = radar.shape
            rect = sv.Rect(x=info.width // 2 - rw // 2, y=info.height - rh,
                           width=rw, height=rh)
    ...
    meta = dict(
        ...
        pitch=dict(length_m=PITCH_LEN_M, width_m=PITCH_WID_M,
